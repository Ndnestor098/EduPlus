        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 container mx-auto">
                    ...
                    <table class="table">
                        ...
                        @foreach ($teachers as $item)
                            <tr>
                                <td><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ $item->name }}</a></td>
                                <td><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ Str::limit($item->email,20) }}</a></td>
                                <td><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ $item->cellphone }}</a></td>
                                <td class="text-center"><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ $item->subject }}</a></td>
                                <td class="text-center"><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ $item->salary }}</a></td>
                                <td class="text-center"><a href="{{route("teacher.admin.edit", ['name'=>$item->name, 'id'=>$item->id])}}">{{ $item->started }}</a></td>
                ...
                <div class="flex justify-center" style="width: 100%; height: 100%; padding: 10px 0px;">
                    <a href="{{route("teacher.admin.add")}}" class="button-search">Agregar Profesor</a>
                </div>
            </div>
        </div>
    </div>
